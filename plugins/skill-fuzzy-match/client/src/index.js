/* @dsh-plugins skill-fuzzy-match — client half
 *
 ...
 *
 * This is a dynamic Cordis client plugin: load it with cordis_define +
 * cordis_run, or paste the body into a session. It monkey-patches the already
 * registered "/skill" source in place (re-registering the same (trigger, name)
 * would throw), reuses the original catalog fetch (empty query returns all
 * skills), then filters/ranks locally with the same fuzzyScore algorithm used
 * by the sibling command source (dsh-client-ui-commands). On stop/update the
 * original candidates is restored.
 *
 ...
 * not be the cleanup fn (a previous version got this wrong and undid the patch
 * immediately).
 */
...
function patchSource(src) {
  ...
  src.candidates = async function (session, req) {
    // Reuse the original fetch path with an empty query to obtain the FULL
    // catalog (startsWith('') returns every skill), then filter/rank locally.
...
return {
  ...
  apply(ctx) {
    ...
    if (attempt()) {
      // ctx.effect runs the callback now and registers its RETURN as disposer.
      ctx.effect(function () { return function () { restoreFn(); }; }, 'skill-fuzzy: restore candidates');
      return;
    }
    // dsh-client-ui-skill may register after us; retry briefly.
    var tries = 0;
